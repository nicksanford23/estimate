...
S3. EVIDENCE PACKET (multi-scale, iterative) — packets BEGIN from room
    anchors but CONSOLIDATE into one packet per candidate
    physical_surface_region (overlapping/open identities merge before
    review). Per surface: full-floor
...
S8. HUMAN GATE — two distinct effects:
    (a) PRODUCT decision: qualified reviewer (founder now; customer
        later) accepts/edits/rejects PHYSICAL_SURFACE_REGIONS ranked by
        measured severity; room and finish identities are reviewed as
        memberships of the surface. One shared surface receives ONE
        geometry decision and ONE training label regardless of identity
        count;
...
Every artifact declares its upstream dependencies. When a source page,
revision, viewport, scale, transform, roster membership, or geometry
changes, ALL affected downstream artifacts (crops, polygons,
measurements, topology results, decisions, exports) become STALE:
retained in history, but unable to be accepted, exported, or used for
training. The project resumes from the earliest changed stage and
regenerates through S10 under a new run/version. Unaffected
floors/surfaces remain valid.
...
- TRAINING pile: many projects, whole-project processing; partial
  coverage acceptable only with TWO complete denominators recorded:
  (1) all discovered room identities + their reconciliation state;
  (2) all discovered physical surfaces + their geometry state. Every
  unprocessed surface carries an explicit state (unresolved / no_plan /
...
## 6. Round-4 blocker dispositions (all accepted)

1 Canonical-unit consistency: S3 consolidates packets per surface; S8
  decides surfaces with identities as memberships; data strategy carries
  dual denominators; one surface = one decision = one label:
  incorporated.
2 Dependency-invalidation rule (§3.5): incorporated.

## 6b. Round-3 blocker dispositions (all accepted)
