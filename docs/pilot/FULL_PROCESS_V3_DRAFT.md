# Full process v3 — DRAFT FOR LOCK (round 3)

Written 2026-07-17 (Claude Fable). Supersedes V1/V2 drafts (kept unchanged).
All 8 Codex round-2 blockers + the S3 crop correction ACCEPTED and
incorporated; dispositions at end. **Codex: respond LOCK or numbered
...
```text
PROJECT FLOW (per project):   S1 -> S1.5 -> S2 -> S3 -> S4 -> S5 -> S5.2
                              -> S5.5 -> S6(loop) -> S7 -> S8 -> S10 export
...
S5.5 MEASURE — reference-confirmed gate:
    ...
    - PASS_MEASURED, exact machine-readable definition — a surface passes
      only when ALL hold:
        ...
        6. S5.2 and S7 gates pass;
        7. no required metric is unknown.
...
T-LOOP (geometry model operations):
    - input: training-eligible surfaces (S8b), versioned dataset
      snapshots, splits by whole projects AND architect/design families;
    - >=150 eligible rooms / >=2 projects / locked label book grants
      EXPLORATORY runs only; model-agnostic bakeoff vs the rented-AI +
      vector baseline; original labels never overwritten;
    - every iteration: fixed validation projects + regression sample of
      previously accepted rooms; sealed exam projects untouched forever;
...
## 6. Round-2 blocker dispositions (all accepted)

1 Training moved out of project flow into async T-LOOP: incorporated (§1, §3).
2 S10 Layer-B estimating/export added: incorporated.
3 M1-LOOP model operations added: incorporated (§3).
4 Complete repair/edit loop incl. S5.2 + S7 return route + S8-edit regating:
  incorporated (S6).
5 Per-edge confirmation mandatory for training eligibility; sampling never
  produces "confirmed" records: incorporated (S5.5, S8b).
6 Scan/raster reference path with second-reviewer confirmation: incorporated
  (S5.5).
7 Exact machine-readable pass_measured definition: incorporated (S5.5).
8 State/handoff contract + reviewer qualification definition: incorporated
  (§4).
S3 iterative crop expansion rule: incorporated (S3).
